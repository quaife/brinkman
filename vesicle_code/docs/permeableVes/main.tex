...
\begin{abstract}
Lipid bilayer membranes have a native (albeit small) permeability for water molecules. Under an external load, a lipid membrane deforms and its
water influx/efflux is often assumed  negligible in the absence of osmolarity, provided 
that the bilayer structure stays intact and does not suffer from poration or rupture. 
%
In this work we use boundary integral simulations to investigate the effects
of native water permeability on the vesicle hydrodynamics due to a mechanical load,  such as the viscous stress
from an external flow deforming a vesicle membrane in free space or pushing it through a confinement. Coupling the 
membrane permeability with membrane elastic stress and tension in the framework of Helfrich free energy, we 
illustrate that, in the absence of an osmotic stress gradient, the model semipermeable vesicle is affected by water 
influx/efflux over a sufficiently long time or under a strong confinement. Our simulation results quantify the conditions 
for water permeation to be negligible in terms of the time scales, flow strength and confinement. These results shed 
light on how microfluidic confinement can be utilized to estimate membrane permeability.
%
...
the water permeability, $R$ is the ideal gas constant, $T$ is the
...
arclength along the membrane, see Supplementary Material). Using
...
For vesicles in a narrow microfluidic channel, the experiments usually do not last longer than a few minutes~\cite{abk-fai-sto2006}.
%\todo[inline]{Howard talked aboutthis statement not always being true}. 
Our simulations show that strong
...
\subsection*{Semipermeable Vesicle in a Narrow Channel (Stenosis)}
 \begin{figure*}[htp]
  ...
  short geometry in (a). (d) The reduced area of the semipermeable
   ...
   \todo[inline]{Figure out the oscillations in excess pressure}}
\end{figure*}
...
\end{align}
%where $\beta = k_w/W$ is the dimensionless water permeability. 
...
When a semipermeable vesicle goes through a contracting channel (slit) the numerical simulations show that both the driving flow and the membrane permeability have to be sufficiently large for a successful passage through the slit, see Figure \ref{fig:contractingComposite}.  As the vesicle goes through the contracting channel, the gradual confinement first leads to a slow inflation. Once the vesicle is close to the neck, it quickly deflates in order to fit the constriction. When the vesicle is near the neck, simulations show that the the water effluxes are in the front and rear of vesicle while most water influx is in the middle. The tension distribution along the vesicle as a function of their position in the contracting channel is summarized in Figure~\ref{fig:contractingTension1}, where we observe the membrane tension has the largest magnitude when the vesicle is at the neck. Away from the neck the membrane tension is quite small due to th
\begin{figure*}[htp]
  \centering
  \begin{tikzpicture}[scale=1]
    \node[inner sep=0pt] at (0,0) {
  ...
  \begin{tikzpicture}[scale=1]
    \node[inner sep=0pt] at (0,0) {
    \includegraphics[width=0.8\textwidth]{figures/contractingTensions2.pdf}
    };
    \node at (-6.79,1.4) {(e)};
  \end{tikzpicture}
  \caption{\label{fig:contractingTensions1} Tension distribution along a
  semi-permeable vesicle going through a contracting geometry. The
  vesicle tension increases by up to five orders of magnitude when as it
  enters the neck. The permeability constant and flow rates are (a)
  $U_{\max} = 5.7\mu$m/s, $\beta = 5 \times 10^{-4}$; (b) $U_{\max} =
