...
\begin{figure}[htp]
  ...
  \end{minipage}
   \caption{\label{fig:InclinationAngle} The
...
Based on how the contact region aligns with the direction of divergent flow, 
we can define the inclination angle of the vesicle doublet as illustrated in Figure~\ref{fig:InclinationAngle}.
%

When $\theta=\pi$ (the long axis of the vesicle doublet is aligned with the far-field diverging flow), the converging flow
...
With the fluid flow pushing the two vesicles towards the stagnation point dynamically placed at the doublet center, 
this configuration is expected to be more stable than the configuration in the middle of Figure~\ref{fig:InclinationAngle} where $\theta=\pi/2$ 
(the long axis of vesicle doublet is parallel to the converging direction).  As shown in the illustration, 
the diverging flow is pulling the vesicles away from each other when $\theta=\pi/2$. 
Thus it is possible that the diverging flow may overcome the adhesion and
break up the doublet. It is also reasonable to expect the vesicle doublet to rotate around the stagnation so the flow is pushing them together rather than pulling them apart.

In the following simulation experiments we place the vesicle doublet with $\theta=\pi/2$ so that the flat contact region is
orthogonal to the diverging flow that is pulling vesicles away from each other.
%In this configuration the flow is pulling the vesicles away from each other, and 
%is expected to be less stable than the $\theta=\pi$ configuration where the fluid flow is constantly pushing vesicles towards each other. 
%
At low extensional
...
= 0.8$ is split with an extensional rate $\chi = 0.1$
(Figure~\ref{fig:extensional4}).  
%
For extensional rates and reduced areas that result in a bound vesicle doublet, 
we observe that the doublet always rotate from the $\theta=\pi/2$ configuration towards
...
%  \end{minipage}
%  \caption{\label{fig:extensionalPhaseDiagram}  A phase diagram of the long-time
