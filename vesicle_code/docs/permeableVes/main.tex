...
\dropcap{W}ater balance is an essential process in the adaptation of a living cell to its environment \cite{AlbertsMolecularBiology}.  
For example, erythrocytes demonstrate efficient exchange of water to participate in efficient body water homeostasis \cite{SugieIntaglietta2018_AmJPhysiolHCP}. Another example is the regulation of cell volume and the cell membrane tension in cell migration \cite{SaadounPapadopoulosWatanabeEtAl2005_JCS}.
The cell membrane is permeable to water and water-soluble molecules \cite{Dick1964_JTB,FettiplaceHaydon1980_PhysRev,DeamerBramhall1986_ChemPhysLipids,Grafmueller2019_ABLS}.
In the presence of osmolarity (a contrast in the concentration of a solute impermeable to the membrane), water flows in or out of the membrane to adjust the interior cellular volume. In the red blood cell membrane, aquaporin-1 (AQP1) channels give rise to a much higher water permeability of $\sim 1.8\times 10^{-2}$ cm/s in erythrocytes \cite{YangMaVerkman2001_JBC} while for an artificial lipid bilayer membrane the water permeability is in the range of $10^{-4}-10^{-3}$ cm/s \cite{ThompsonHuang1966_ANYAS,FettiplaceHaydon1980_PhysRev,Grafmueller2019_ABLS,Dimova2020_GVB}.

Vesicles (self-enclosed lipid bilayer membranes) have been used as a model system to study reshaping, remodeling and scission of cell membranes due to osmotic stress \cite{OgleckaEtAl2014_eLife,CamposSaric2020_bioRxiv,Dimova2020_GVB}. Under an osmotic shock, a vesicle may undergo oscillatory variation in its enclosed volume \cite{ChabanonHoLiedberg2017_BJ}. 
In these situations, the osmolarity is the main drive for water transport across the membrane. Thus most modeling works ignore the mechanical effects (capillary pressure, bending force and membrane tension) on vesicles under an osmotic stress. 
However, the osmotic water flux is inevitably coupled with the mechanical pressure jump across the membrane\cite{yao-mor2017}, which could become comparable to osmotic stress when the membrane deformation is large \cite{LiMoriSun2015_PRL,yao-mor2017}.
...
Based on our analysis, a semi-permeable vesicle in a quiescent flow
...
proportional to $\beta$, so we expect the times for the vesicle to
deviate from its initial reduced area and to reach its steady state to
both be proportional to $\beta^{-1}$. This is verified by considering a
vesicle with initial reduced area $\nu = 0.65$ and several different
semi-permeability rates. In Table~\ref{tbl:ellipseRelaxTimes} we report
the time, $t_\mathrm{steady}$, required for the vesicle's area to be
within 1.3\% of its steady state reduced area, and the time,
$t_\mathrm{inflate}$, required for the vesicle's reduced area to exceed
...
dynamics of four vesicles $\beta = 10^{-3}$ and a variety of initial
configurations.  First, in all cases the vesicle eventually reaches a
circular shape of reduced area $\nu=1$. Second, we observe that even
though the vesicle is inflating to a circle, meaning that there is a net
flux into the vesicle, there are regions where the flux is out of the
vesicle. These regions are denoted in blue and are only present at early
times.  Finally, the vesicles area does not initially change by much,
and intermediate shapes of semi-permeable vesicles resemble the shape of
an impermeable vesicle. This can be explained by considering the two
terms that contribute to the vesicle velocity: (i) the permeability
velocity, $\beta(\ff \cdot \nn) \nn$, and (ii) the force balance
velocity, $\SS[\ff]$. Since $\beta = 10^{-3}$, the permeability velocity
is much smaller than the force balance velocity at early times, and the
semi-permeable vesicle dynamics resembles those of an impermeable
vesicle. As the force balance velocity decreases, the permeability
velocity dominates, and the vesicle begins to inflate to a circle with
no vanishing force and velocity. 
...
or tumble~\cite{fin-lam-sei-gom2008, kra-win-sei-lip1996}. In our
experiments, tumbling is impossible since we consider semi-permeable
vesicles with no viscosity contrast. Similar to the dynamics of an
impermeable vesicle, a semi-permeable vesicle tilts to an inclination
angle and undergoes tank treading dynamics.

In the quiescent flow, the steady state vesicle shape is independent of
the initial vesicle shape, and we investigate if this is true when a
shear flow is imposed. In Figure~\ref{fig:shearComposite}(a), using four
initial reduced areas and two flow rates, we plot the reduced area of a
semi-permeable vesicle with $\beta = 10^{-3}$. As we saw for the
quiescent example, the final shape does not depend on the initial
reduced area. However, it does depend on the flow rate. We also observe
a very small change in the reduced area at early times with the vesicle
dynamics at early times being similar to those of an impermeable
vesicle.  However, once the vesicle is tank-treading, the normal
component of the force-balance velocity is small and the permeability
velocity dominates and leads to a changing reduced area.

We also form the steady state shape for a variety of semi-permeability
rates and flow rates. Since the final shape is independent of the
initial reduce area, we choose an vesicle with $\nu = 0.65$ for all the
simulations. The steady state vesicle shape, along with its reduced
...
semi-permeability rate has no significant effect. Instead, as we saw in
...
\begin{figure}[htp]
  ...
  an impermeable vesicle submerged in a Poiseuille flow with varying
  initial reduced areas and flow rates. The results agree with Kaoui et
  al.~\cite{kao-bir-mis2009}. (b) The steady state shape of a
  semi-permeable vesicle submerged in a Poiseuille flow with varying
  initial reduced areas and flow rates.}
\end{figure}
...
%\begin{figure}[htp]
%%  \ifTikz
%%  \input{figures/stenosisGeom.tikz}
%%  \fi
%  \includegraphics[width=\linewidth]{figures/stenosisGeom.pdf}
...
%\begin{figure}[htp]
%%  \ifTikz
%%  \input{figures/stenosisPressureFluctuation.tikz}
%%  \fi
%  \includegraphics[width=\linewidth]{figures/stenosisPressureFluctuation.pdf}
