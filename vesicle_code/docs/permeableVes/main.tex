...
For a semipermeable vesicle without confinement, our simulations show
that the dynamic evolution towards equilibrium is dictated by the
background flow field, while the permeability $\beta$ only rescales the
time in the dynamics. With confinement, we find that we need to use
larger values of $\beta$ to keep the membrane tension below the poration
value $\sim 10^{-3}$ N/m. Such large values of $\beta$ may pertain to a
lipid bilayer membrane with aquaporin proteins at a high temperature.
...
hydrodynamics only after a duration of $\sim 1$ day. For vesicles in a
narrow microfluidic channel, the experiments usually do not last longer
than a few minutes~\cite{abk-fai-sto2006}. Our simulations show that
strong confinement can lead to amplification of water permeation at a
time scale shorter than a few minutes.
%short time scales.
