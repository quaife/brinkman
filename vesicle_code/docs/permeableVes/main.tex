...
\dropcap{W}ater exchange is essential as a living cell adapts to its environment \cite{AlbertsMolecularBiology,YangMaVerkman2001_JBC,SugieIntaglietta2018_AmJPhysiolHCP,SaadounPapadopoulosWatanabeEtAl2005_JCS,Verkman2008_JMM,BerthaudEtAl2016_SM,Keren2011_EurBJ,TaloniKardashSalmanEtAl2015_PRL}.  
%
%Water permeation across the cell membrane due to an osmotic shock has been well studied \cite{AlbertsMolecularBiology}. 
Recent findings suggest that both osmotic and mechanical stresses can induce water permeation that is significant enough for cells to migrate under strong confinement \cite{StrokaJiangChenEtAl2014_Cell,LiMoriSun2015_PRL,yao-mor2017}, leading to a dynamic surface-to-volume ratio of the migrating cell. 
...
mechanically induced water permeation is often associated with membrane poration or rupture under extreme stresses \cite{RazizadehNikfarPaulLiu2020_BJ}. In this work we use modeling and state-of-the-art numerical simulations to show that the intrinsic membrane permeability to water can give rise to significantly distinct hydrodynamics of a semipermeable vesicle without poration or rupture in the membrane. 
%
%
...
simulations show that a stretched RBC membrane may porate when membrane tension reaches the order of $2-4$ mN/m (\cite{RazizadehNikfarPaulLiu2020_BJ} and references therein), giving rise to enhanced membrane permeability to both water and macromolecules. Consequently under conditions where membrane remains intact (no poration or rupture), RBCs and vesicles are assumed to be of the same volume throughout the experiments no longer than tens of minutes. 
%
...
Under what condition is this a good assumption? What is the dynamic consequence when permeable water flux affects the membrane hydrodynamics?
...
\end{figure*}
\todo[inline]{put a vertical dashed line at the times when permeability
starts to be important. Convert to physical time.}
...
channel, see Figure~\ref{fig:stenosisComposite}(c). A constant
Poiseuille velocity with a maximum value of $1000\mu$m/s is maintained
...
that less excess pressure is required to maintain a constant flow rate
when the vesicle is semi-permeable. First we note that the vesicle
...
the vesicle enters the stenosis channel, the vesicle continues to
deflate in the front even though we observe influx (red segment) at
various rear locations in the membrane.  These results are consistent
with the results in~\cite{Pak2015_PNAS} where the membrane tension is
sufficiently large in the front of the vesicle as it goes through a
constrictive channel.

Our simulations indicate the semi-permeability does affect the excess
pressure, and the vesicle's reduced area reduces by about 6.1\%
...
through a channel of double the length, and we observe that the reduced
area decreases by 13.2\% (Figure~\ref{fig:stenosisComposite}(e)).
\todo[inline]{This example also used a smaller flow rate of $8.4\mu$m/s}
\todo[inline]{Membrane tension. Look at each of the five cases and find
the instance where tension reaches its max value. Plot the tension as
the colorbar along the vesicle shape.}
...
The reduced area in Figure~\ref{fig:contractingComposite}c and f shows that the vesicle always inflates gradually
before it reaches the neck, where vesicle quickly deflates.  
...
\begin{itemize}
  ...
  \item Vesicle velocity in plot (b) varies from 5 to 28 microns/s
  \item Vesicle velocity in plot (c) varies from 0.2 to 2 microns/s
  \item Vesicle velocity in plot (d) varies from 5 to 25 microns/s
  \item Vesicle velocity in plot (g) varies from 14 to 95 microns/s (see
    plot (i))
\end{itemize}
\todo[inline]{Center of mass vs.~time}
\todo[inline]{Membrane tension. Look at each of the five cases and find
the instance where tension reaches its max value. Plot the tension as
the colorbar along the vesicle shape.}
