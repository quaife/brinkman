...
In Figure~\ref{fig:relaxationArea}, we examine the transient dynamics of
four vesicles $\beta = 10^{-3}$ and a variety of initial configurations.
First, in all cases the vesicle eventually reaches a circular shape of
reduced area $\nu=1$. Second, we observe that even though the vesicle is
inflating to a circle, meaning that there is a net flux into the
vesicle, there are regions where the flux is out of the vesicle. These
regions are denoted in blue and are only present at early times.
Finally, the vesicles area does not initially change by much, and
intermediate shapes of semi-permeable vesicles resemble the shape of an
impermeable vesicle. This can be explained by considering the two terms
that contribute to the vesicle velocity: (i) the permeability velocity,
$\beta(\ff \cdot \nn) \nn$, and (ii) the force balance velocity,
$\SS[\ff]$. Since $\beta = 10^{-3}$, the permeability velocity is much
smaller than the force balance velocity at early times, and the
...
\begin{figure*}[htp]
  \centering
  \input{figures/relaxationArea.tikz}
  \caption{\label{fig:relaxationArea} (a): Snapshots of four
...
the initial vesicle shape. Here we test if a similar results holds with
an imposed flow. In Figure~\ref{fig:shearSS}, we plot the reduced area
of four vesicles with different initial reduced areas. The final shape
depends on the initial vesicle length, but not on its initial reduced
area. Therefore, for this example we only consider vesicles with the
initial reduced area $\nu = 0.65$. For all permeability rates and shear
rates, the semi-permeable vesicle tilts to an inclination angle and
undergoes tank treading dynamics. Because of the semi-permeability, the
initial and final reduced areas differ. In
