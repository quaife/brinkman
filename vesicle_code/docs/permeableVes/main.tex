...
Furthermore, we note that in the absence of an osmotic stress, the permeating water
flow may be non-negligible when the membrane tension becomes large due
...
For a semipermeable vesicle without confinement, our simulations show that the
dynamic evolution towards equilibrium is dictated by the background flow field, while the permeability $\beta$ only rescales the time in the dynamics.
With confinement, we find that we need to use larger value of $\beta$ to keep the membrane tension below the poration value $\sim 10^{-3}$ N/m. 
Such large value of $\beta$ may pertain to 
a lipid bilayer membrane with aquaporin proteins at a high temperature.
...
permeability may lead to different vesicle hydrodynamics only after  a duration of $\sim 1$ day. For vesicles in a narrow microfluidic
...
three cases efflux is dominated by influx, especially after $t=1$ day when
the increase in total water content becomes non-negligible.
%
...
%
Over time (about four weeks) all
...
k_w \mu /R_0$.  
We convert the apparent permeability of 
a polyunsaturated PC bilayer  \cite{OlbrichRawiczNeedhamEtAl2000_BJ}
to the hydraulic conductivity $k_w$ using the conversion in \cite{FettiplaceHaydon1980_PhysRev}, and  found that 
$10^{-14}\le k_w\le 10^{-12}$ m$^2$s/kg, thus $\beta$ is  in the range $10^{-11}\le \beta\le 10^{-9}$.
