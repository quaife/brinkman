...
\begin{abstract}
The dynamics of adhesive vesicles under various flowing conditions is
...
fluid between them to form a doublet at equilibrium. A lubrication
analysis on such draining of the thin film gives the dependencies of
...
above which adhesive interaction is overcome by the hydrodynamic
...
is close to $\delta$ and large deformation occurs in the ``contact region". The L.-J. type potential prohibits
physical contact between two membranes and instead keep them close to the separation distance $\delta$.
In Section~\ref{subsec:qflow_draining_times}, we
...
\begin{figure}
  ...
  \caption{\label{fig:Dec18_vesicle_shape} Equilibrium configuration of
  a doublet of identical vesicles under adhesion in a quiescent flow.
  Vesicle reduced area $\Delta A=0.95$, $0.75$, and $0.6$ are labeled.
  The color coding is the tension along the vesicle.  Vesicle length is
  fixed at $2\pi$, (dimensionless) bending rigidity modulus $\kappa=1$,
  $\mathcal{H}=5$, and $\delta=0.2$.  {\em Left}: Overall equilibrium
  shapes of two vesicles under adhesive interactions vary with reduced
  area.  {\em Right}: Membrane shapes in the contact region.} 
\end{figure}
...
decreases further, we observe undulation of the 
vesicle membrane on the non-contact side while the contact region remains
...
\includegraphics[keepaspectratio=true,scale=0.175]{figs/Dec18_EbEadh_vs_rA_adR0p2_adS502.jpeg}
  \caption{\label{fig:Dec18_vesicle_equilibrium1}   {\em Left}:
  ...
  of two energies plotted against reduced area $\Delta A$. Two vesicles in the doublet are of the same length, reduced area
  and bending rigidity modulus ($\kappa=1$) with $\mathcal{H}=5$ and $\delta=0.2$.}
\end{figure}
...
\includegraphics[keepaspectratio=true,scale=0.18]{figs/Dec18_Eadh_vs_sigma_rA0p9502.jpeg}
  ...
  Total bending energy ({\em left}) and adhesion energy ({\em right}) of
  a vesicle doublet at equilibrium versus the adhesion strength
  $\mathcal{H}$.  The two vesicles in the doublet have the same length
  and reduced area $\Delta A=0.95$, with separation distance $\delta =
  0.2$ (triangles), $0.3$ (crosses), and $0.4$ (circles).}
\end{figure}
...
point placed at the center of the vesicle doublet.  
When the long axis of the vesicle doublet is aligned with the far-field diverging flow, the converging flow
pushes vesicles towards each other and the repulsive interaction is essential to keep the two vesicles at the separation distance.
In this configuration the extensional flow is pushing the vesicles together. With the stagnation point placed at the center of the doublet, this
configuration appears to be quite stable for low extensional flow rate.

However, when the long axis of vesicle doublet is parallel
to the far-field converging flow, the diverging flow may overcome the vesicle adhesion to
split the two vesicles apart. Thus in the following simulation experiments we place the vesicle doublet so that the flat contact region is
aligned with the axis of converging flow. In this configuration the flow is pulling the vesicles away from each and 
is expected to be less stable than the orthogonal configuration where the fluid flow is constantly pushing vesicles towards each other. 
It is also reasonable to expect the vesicle doublet to rotate around the stagnation so the flow is pushing them together rather than pulling them apart.


At low extensional
rates, we expect the vesicle doublet to stay at the fluid
...
In Figures~\ref{fig:extensional1}--\ref{fig:extensional3},
...
\begin{figure}[htp]
\todo[inline]{Figures of tension and adhesion of the final
configuration}
...
Institute, part of the Simons Foundation.
