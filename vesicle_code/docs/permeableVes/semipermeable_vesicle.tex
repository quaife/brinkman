...
permeability coefficient has a very minor effect on the equilibrium shape for $10^{-4}\le\beta\le 10^{-2}$. Instead, 
...
\begin{figure*}[htp]
  ...
  \begin{tikzpicture}[scale=1]
    ...
    \node at (+5.0,1.5) {(d)};
  \end{tikzpicture}
  \todo[inline]{Overflow. Need to reduce sizes. Convert to tikz}
...
\subsection{Semipermeable Vesicle in a Closely-fit Channel (Stenosis)}
 \begin{figure*}[hbp]
  ...
  \fi
   \todo[inline]{Overflow. Need to reduce sizes}
...
%  \fi
  \todo[inline]{Do max tension vs. center of mass and time for plot
  (d).}
