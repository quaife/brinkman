...
%
\includegraphics{head_foot/dates} & \noindent\normalsize{We numerically study the hydrodynamics of multicomponent vesicles in strong confinement as a simplified model for red blood cells going through extreme constriction.  Under strong confinement, we parametrize the bending modulus so that it is positive for all values of the lipid phase parameter. For a multicomponent vesicle going through a stenosis, we find the correlation between the lipid phase coarsening and the lubrication layer (such as size and flow profile), excess pressure, and tank treading in the membrane. For a multicomponent vesicle going through a contracting channel, we find that the lipid always phase separates so that the vesicle is stiffer in the front as it passes through the constriction.  For both cases of confinement we find that the lipid coarsening is arrested under strong confinement, and proceeds with high rate upon relief from extreme confinement. These results may be useful for efficient sorting lipid domains using microfluidic flows by controlled release of vesicles going through strong confinement. } \\%The abstrast goes here instead of the text "The abstract should be..."
%
\end{tabular}
%
...
Section~\ref{sec:results}, we calculate and analyze the flow
inside the lubrication layers between the vesicle and the channel walls to elucidate the correlation between hydrodynamics and membrane dynamics.
