...
(contracting). 
%We considered a semipermeable vesicle suspended in these
%same geometries in our previous work~\cite{qua-gan-you2021}. 
...
-\sigma_s$ which is the Marangoni stress.  For a single-component vesicle, the streamwise gradient 
...
between two lipid domains, where the membrane energy changes sharply between lipid species. This implies that, by visualizing the flow
in the lubrication layer, it is possible to identify the location of boundaries between lipid domains as shown in Figure~\ref{fig:BLvelocities}(b).
