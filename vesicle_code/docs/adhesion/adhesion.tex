...
The dynamics of adhesive vesicles under various flowing
...
flow, two nearby vesicles move slowly towards each other under the adhesive
potential, pushing out fluid between them to form a doublet at
...
<<<<<<< HEAD
engineering \cite{sackmann1996,Barthes-Biesel2016_ARFM}.  The ever expanding applications have
encouraged novel experimental, theoretical, and numerical
...
>>>>>>> 7c0a854d86f240e04cf8d3b46da215f6da53b14c
...
bending rigidity modulus and $s$ is arclength.  For the numerical examples, we
set $\kappa_b = 1$ and consider adhesion strengths ranging from $0.1$ to
$1$ times the bending rigidity modulus.  The stretching term is $\TT \sigma  =
...
Sukumaran and Seifert~\cite{suk-sei2001} considers
...
<<<<<<< HEAD
  \caption{\label{fig:Dec18_vesicle_shape} Equilibrium configuration of a doublet of 
  identical vesicles under adhesion in a quiescent flow. Vesicle reduced area $\Delta A=0.95$, $0.75$, and $0.6$
  are labeled.  The color coding is the tension along the vesicle. 
  Vesicle length is fixed at $2\pi$, (dimensionless) bending rigidity modulus $\kappa=1$, $\mathcal{H}=5$, and $\delta=0.2$.
  {\em Left}: Overall
  equilibrium shapes of two vesicles under adhesive interactions vary with reduced area.  
  {\em Right}: Membrane shapes in the contact region.} 
%  \todo[inline]{This
%  caption contradicts itself.  Is it $\mathcal{H} = 0.5$ or $\mathcal{H}
%  = 5$?}
=======
  \caption{\label{fig:Dec18_vesicle_shape} Two vesicles of the same length, reduced area
  and bending rigidity ($\kappa=1$)
  with $\mathcal{H}=5$, and $\delta=0.2$.  {\em Left}:
...
\includegraphics[keepaspectratio=true,scale=0.18]{figs/Dec18_Eadh_vs_sigma_rA0p9502.jpeg}
  ...
  Total bending energy ({\em left})
  and adhesion energy ({\em right}) of a vesicle doublet at equilibrium versus the adhesion strength
  $\mathcal{H}$.  The two vesicles in the doublet have the same length and reduced area $\Delta A=0.95$, with 
...
<<<<<<< HEAD
and found it to be more than twice the effective shear viscosity of a dilute suspension of single vesicles.
Furthermore, we find that the membrane adhesion contribution to the shear viscosity increases with decreasing
reduced area while the bending/tension contribution increases with the reduced area.
%
%because deflated vesicles (low reduced area) deform more
%easily and more membranes in the contact region.
%\todo[inline]{I don't understand this last sentence --> modified now by yny}
=======
