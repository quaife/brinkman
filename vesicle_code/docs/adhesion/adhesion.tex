...
\begin{figure}[htp]
  ...
  \caption{\label{fig:extensionalNonsymmetric} }
  \end{figure}
...
\end{figure}
\todo[inline]{Redo this image with Hamaker constant 2.1 (or 2.2) which
has a comparable period so that we can see the different dynamics}
...
$\Delta A = 0.9$.  In
Figure~\ref{fig:sflow_distance}(a), we plot the minimum distance
...
doublet to make a complete revolution at a variety of Hamaker constants
and shear rates.  For a fixed Hamaker constant, as expected, the period
decreases for larger shear rates.  In addition, for a fixed shear rate,
the period decreases as the Hamaker constant increases, but the period
eventually plateaus.  The dynamics of these doublet have similar
behavior to a single rigid body undergoing a Jeffery orbit in a shear
flow.  Therefore, we define the effective aspect ratio
$\lambda_{\mathrm{eff}}$ which satisfies
\begin{align*}
