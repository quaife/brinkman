...
doublet.  We also plot the time required for the doublet to make a
complete revolution at a variety of Hamaker constants and shear rates.
For a fixed Hamaker contant, as expected, the period decreases for
larger shear rates.  In addition, for a fixed shear rate, the period
decreases as the Hamaker constant increases, but the period eventually
plateaus.  More interesting, for larger shear rates, the period of the
doublet appears to be independent of the Hamaker constant.
...
\begin{figure}
  ...
  $\mathcal{H}=0.1$ and $\mathcal{H}=1.0$.  {\em Right}: The time
  required for a doublet to complete a single rotation.  The reduced
  area is $\Delta A = 0.90$ and the separation distance is $\delta =
  0.4$.  The different lines correspond to different shear rates.}
\end{figure}
...
%  \includegraphics[width=0.45\textwidth]{figs/doublet_decomp.pdf}
  ...
  the bending and tension (blue) of the vesicles, and the contribution
  from the vesicle adhesion (red).  Also included is the intrinsic
  viscosity of a single tank-treading vesicle (dashed black line).}
\end{figure}
