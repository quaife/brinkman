...
right vesicle (elevated with a small vertical displacement at $t=0$) further away, causing rotation of vesicles as a pair. 
...
For strong adhesion (such as the cases with ${\cal H}=0.7$ in Figure \ref{fig:extensionalNonsymmetric}),
 the two vesicles rotate and move away from the stagnation point as a doublet even for $\triangle y=0.1$, where
 two vesicles are in close vicinity of each other only at the tip at $t=100$ in the third row.
For weaker adhesion (${\cal H}=0.6$ in Figure \ref{fig:extensionalNonsymmetric})  
the two vesicles no longer form a doublet as they are quite separate from each other at $t=100$.

 with an initial vertical displacement $\triangle y = 0.1$ no longer form a doublet
and are separate from each other at the end of simulation ($t=100$).
...
A small initial vertical displacement $\triangle y$ of one vesicle causes both vesicles to not only rotate but also move away from the
center. Thus in the microfluidic experiment it is often more desirable to trap the vesicle around the center of an extensional flow \cite{Spjut2010_MSThesis_Chapter3}.
...
>{\centering\arraybackslash}p{2cm}}
  ...
  1.00 & 19.47 & 3.00 
\end{tabular}
\todo[inline]{Update these values with new periods.  Maybe need to
consider more than one Hamaker constant}
