...
bending stiffness. 
\textcolor{blue}{
To rectify this issue, we could simply restrict the bending modulus to
be bounded in $[b_{\min},b_{\max}]$ by letting $b(u) = \max\left( \min
\left( (1-u) + \beta u,1 \right), \beta \right)$. However, the loss of
differentiability would result in a loss of accuracy.
}
...
\textcolor{blue}{
where we use the parameter value $\delta = 0.3$.
}
...
\textcolor{blue}{
with a fixed dimensionless length of $L = 7.4$
}
...
\textcolor{blue}{
The dimensionless width of the stenosis starts at $6.8$ and narrows to
$1.4$. 
}
With the current setup and $Ca=0.25$, an elliptical vesicle with reduced
...
slowly contracts 
\textcolor{blue}{
from a dimensionless width of $6.5$ to $0.63$,
}
