...
bending rigidity modulus and $s$ is arclength.  For the numerical
examples, we set $\kappa_b = 1$ and consider adhesion strengths ranging
from $0.1$ to $1$ times the bending rigidity modulus.  The stretching
term is $\TT \sigma  = (\sigma \xx_s)_s$, where $\sigma$ is the tension,
and the tension acts as a Lagrange multiplier to satisfy the local
...
term, $\AA \xx$, that we now describe.
