...
StokesMat = op.StokesMatrixLogless(X);
...
sigma1 = k(1:N)*c1 - LogKernel1*c2 + X(N+1:end)*o.shearRate;
...
tracJump = [[lambTil.*cur.*sin(theta)-dlamTilds.*cos(theta)/L];...
            [-lambTil.*cur.*cos(theta)-dlamTilds.*sin(theta)/L]];
...
un = k(1:N)*c1 - tau(1:N)*c2 + X(N+1:end)*o.shearRate;
