...
the hydraulic permeability, $R$ is the ideal gas constant, $T$ is the
...
Material). Using $k_w\sim 10^{-12}$ m$^2$s/kg (typical of mammalian cell
membranes~\cite{LiMoriSun2015_PRL}) and an osmolarity $\triangle c =
0.1$ mol/L (consistent with the osmotic forces in vesicle aspiration
...
three cases efflux is found at some locations of high curvature, while
mostly water permeates into the vesicle and significant change in the
reduced area is found at $t\sim 1$ hour. Over time (about ten days) all
...
characterized by the elastic capillary number $Ca_\mathrm{E}=\chi\tau$,
where $\tau$ is the characteristic time scale in the
non-dimensionalization (see Supplementary Material) and $\chi$ is the
...
As observed for the planar shear flow, the equilibrium vesicle reduced area is
independent of the initial reduced area and $\beta$, and  larger maximum flow velocity $U$ results in
smaller equilibrium reduced areas, as summarized in Figure~\ref{fig:parabolicComposite}(a). At
the smallest velocitty $U= 200\; \mu$m/s, the steady state reduced
...
\begin{figure}[hbp]
  ...
  through the contraction depends on the semipermeability rate. (d--f):
...
When cells go through small capillary vessels, they experience
large shape deformation and significant membrane stretching that leads to release of messengers 
%Cells experience stretching and release messengers when they pass
%through capillary vessels
~\cite{Wan2008_PNAS, ForsythWan2011_PNAS,Pak2015_PNAS}. 
...
1000 \;\mu$m/s at the inlet and outlet and is chosen so
...
\acknow{We thank Howard Stone for suggestions. B.Q.~acknowledges support
...
\begin{align}
  ...
  \xx \in \gamma,
\end{align}
where $k_w$ is the hydraulic permeability.
...
$k_b/R_0^2 = 10^{-7}$ N/m, and the permeability by $R_0\mu = 2 \times
