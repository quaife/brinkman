...
$\rr = \xx - \yy$, $\rho = |\rr|$,
\textcolor{blue}{
$\nn$ is the outward unit normal, $\eeta$ is a currently unknown density
function,}
...
\begin{figure*}[h]
  ...
  \textcolor{blue}{The background flow is in the positive $x$ direction
...
chosen randomly from the uniform distribution with the range $[0,1]$. As
a result of selecting $b_{\min}=0.1$ and $b_{\max}=1$, the mean bending
stiffness is $\overline{b(u)} = 0.55$.}
...
stenosis. 
\textcolor{blue}{
In contrast, the bending and tension energies of the floppy
single-component vesicle evolve throughout the entire channel as the
vesicle undergoes deformations.}
