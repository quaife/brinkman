...
\authorcontributions{B.Q.~and Y.-N.Y.~designed and perfored research
...
simulations of a semipermeable vesicle in these flows show that water
permeability may lead to different vesicle hydrodynamics over a time
scale of 10 to 100 minutes. For vesicles in a narrow microfluidic
...
$\uu_{\infty}(\xx) = \dot{\gamma} (y,0)$.
 %with different permeability $\beta$ and shear rates $\dot{\gamma}$. 
...
1000 \;\mu$m/s is maintained at the inlet and outlet and is chosen so
...
We consider a two-dimensional inextensible semipermeable vesicle membrane $\gamma$
suspended in a viscous Newtonian fluid in domain $\Omega$.
The vesicle membrane $\gamma$ is parameterized as $\xx(s,t)$ where $s$ is arclength and $t$ is
time. In the suspending fluid the flow velocity $\uu$ and pressure $p$ are governed by the incompressible
Stokes equations
\begin{align}
  \label{eqn:governing}
...
\end{align}
where $\mu$ is the fluid viscosity.  When $\Omega$ is an unconfined geometry, the
condition $\uu(\xx) \rightarrow \uu_\infty(\xx)$ as $|\xx|
\rightarrow \infty$ is enforced. When $\Omega$ is a confined geometry, the boundary condition
$\uu(\xx) = \UU(\xx)$ is enforced on the domain boundary $\partial \Omega$, where $\UU(\xx)$
is prescribed. Along the vesicle membrane, mass continuity, force balance, and
local inextensibility are enforced on $\gamma$ by requiring
\begin{align}
  \label{eqn:jump}
  \jump{\uu} = 0, \quad
...
the local inextensibility condition, but the enclosed area is not conserved
...
%Away from the neck the tension is relatively small, and water efflux is found in the corner of the vesicle.
%\todo[inline]{I might have accidentally deleted some of your work here.Sorry!}
\begin{figure}[htp]
  \centering
  \begin{tikzpicture}[scale=1]
    \node[inner sep=0pt] at (0,0) {
