...
the contact zone to buckle under strong adhesion.  A direct consequence
of the buckling is the sigmoidal shape of the adhesion region, which is
...
We consider a suspension of locally inextensible vesicles in an unbounded
two-dimensional viscous fluid.  For simplicity, we assume the fluid
viscosity both inside and outside the vesicles is the same; however,
incorporating viscosity contrast is rather straightforward in our
numerical algorithm.  
While we focus on a vesicle doublet suspension for the rest of the paper, here we provide the most general formulation for multiple vesicles interacting with
each other through both hydrodynamic and adhesive forces.
Individual vesicles are denoted as $\gamma_j$,
$j=1,\ldots,M$, and they are parameterized in arclength as $\xx_j(s,t)$.
The union of all vesicles is denoted by $\gamma$.  Given a background
velocity $\uu_\infty$, the governing equations are
\begin{equation}
\begin{aligned}
...
Adhesion potential for a vesicle membrane has been modeled by a L.-J. type potential
...
\end{align}
where $\mathcal{H}$ is the Hamaker constant and $\delta$ is the adhesion length
scale. $z$ is the distance between a patch on the vesicle membrane and a point on the other object,
which could be another vesicle membrane or a flat solid wall.  The characteristics of the L.-J. potential are summarized in
the left panel of Figure~\ref{fig:adhesionModel}: The adhesive force is zero at the equilibrium distance $z=\delta$.
For large distances ($z> \delta$) the adhesive potential is attractive while for small distances ($z < \delta$) the potential is repulsive to prevent physical contact.

The exponents $(m,n)$ depend on the geometry and the molecular details of the two objects under
adhesion~\cite{Book_IntermolecularSurfaceForces}: $(m,n)=(4,2)$
corresponds to two flat, planar surfaces interacting with each other, and has been a common choice 
for studying membrane-solid adhesion  \cite{Seifert1990_PRA,suk-sei2001,ShiFengGao2006_ActaMechSin, LinFreund2007_IntJSolidsStructures,BlountMiksisDavis2013_PRSa,YoungStone2017_PRF}.
%
On the other hand,
$(m,n) = (12,6)$ corresponds to the Lennard-Jones (L.-J.)~potential
between two molecules, and has been used to model membrane-membrane adhesion  \cite{FlormannAouane2017_SciReports}.  
In our case, the adhesion potential in the
integral of equation~(\eqref{eqn:adhesionForce}) is between two small
patches of lipid bilayer membrane (because the lipid molecules are
coarse-grained in the continuum modeling).  
Thus a reasonable choice for
$(m,n)$ between two coarse-grained membrane patches would be between
those for two planar surfaces and two point molecules.
...
two vesicles forming a doublet in a quiescent flow and in a shear flow.  We found that, as long as the
close-range interaction is well resolved both in space (using the near-singular evaluations) 
and time (using the high-order adaptive time integration), there is very
little difference in both the dynamic evolution and equilibrium
configuration between $(m,n)=(8,6)$ and $(m,n)=(4,2)$.  Therefore, in
this work, we use $(m,n) = (4,2)$ to regulate the numerical stiffness introduced
by the adhesive force. 
%
%For some simulations we used both sets of $(m,n)$ and did not notice significant difference
%in terms of adhesive hydrodynamics of vesicles.

Focusing on intermediate adhesive strengths, 
we use Hamaker constants ranging from $\mathcal{O}(10^{-1})$ to
$\mathcal{O}(10^0)$ times the bending rigidity modulus.  
We assume that the adhesion force from equation~\ref{eq:adhesion_potential} applies
between all pairs of points on different vesicles, and in
Appendix~\ref{sec:appendixA} the net adhesive force at a point $\xx$ on
...
\end{align}
This summation of adhesive forces is illustrated in the middle panel of Figure~\ref{fig:adhesionModel}. 
The right panel of Figure~\ref{fig:adhesionModel} is an 
example of the calculated adhesive force projected onto their center-of-mass vector. We notice that 
the adhesive force is repulsive in the contact region, while for the rest of the membranes there is an attractive force
that acts to pull the two vesicles together. The summary of boundary integral formulation can be found in Appendix \ref{sec:AppendixB} and
...
$2\pi$.  Figure~\ref{fig:doublet090} shows snapshots of two vesicles
...
\begin{figure}
  ...
  \caption{\label{fig:sflow_phase_diagram} {\em Left}: Distance between
  a pair of vesicles in a planar shear flow with shear rate $\chi=0.5$,
  and separation distance $\delta = 0.4$.  The different lines
  correspond to a linear spacing of Hamaker constants ranging from
  $\mathcal{H}=0.1$ and $\mathcal{H}=1.0$.  {\em Right}: Phase diagram
  of vesicle hydrodynamics in a shear flow.  At a fixed reduced area
  $\Delta A=0.90$ and $\delta = 0.4$, the critical Hamaker constant
...
vesicles, and $\langle \cdot \rangle$ is the spatial average.  In the
left plot of Figure~\ref{fig:shearIntrinsicViscosity}, we compare the
intrinsic viscosity of a single tank-treading vesicle to a doublet.  To
validate our simulations, we superimpose (black marks) the intrinsic
...
to right, the plots are the initial condition, streamlines at an
intermediate time, tension at the steady-state configuration, and the
adhesion force projected onto the outward unit normal at the
...
adhesion to keep vesicles bound under linear flows?  Recently Liu {\em
...
modeling and simulations of adhesive membranes with thermal fluctions.
...
NSF-DMS 1614863 and 1412789.  The work of SV and YNY was also supported
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{The Shear and Normal Stresses}
\label{sec:AppendixC}
...
\end{align*}
where $\rr = \xx - \yy$, and $\rho = \|\xx - \yy\|$.  To compute the
stress tensor on a vesicle, there is a jump in the layer potential
$\SS_T$, and the stress tensor is
\begin{align*}
