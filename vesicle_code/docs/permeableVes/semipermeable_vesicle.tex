...
\begin{figure*}[htp]
  \centering
  \ifTikz
  \input{figures/parabolicTensions.tikz}
  \else
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{The Role of Tension and Bending on Semipermeability}
\textcolor{magenta}{Since $\jump{p}= \ff_\mathrm{mem} \cdot
\nn$~\cite{poz1992}, our model assumes that the permeating flux is
proportional to the mechanical pressure jump across the membrane. We
decompose the normal component of the membrane force into its
...
After multiplying~\eqref{eqn:areaROCdecomposed} by $4\pi/L^2$, where $L$
is the constant  vesicle length, the rate of change of the reduced area
due to the tension and bending forces are formed. We investigate the
...
\textcolor{magenta}{Using equation~\eqref{eqn:areaROCdecomposed}, we
investigate the effects of the tension and bending forces on the water
flux through the membrane. In Figure~\ref{fig:stenosisRAdecomp}, we
compute the rate of change of the reduced area due to the tension force
(blue), the bending force (red), and the total membrane force (black).
In both the short and long closely-fit channels, the tension force is
responsible for nearly all the semipermeability. This is in contrast to
the unconfined examples where the tension and bending forces were
each responsible for a comparable amount of semipermeability.}

\begin{figure*}[htp]
  \ifTikz
  \input{figures/stenosisRAdecomp.tikz}
  \else
...
\textcolor{magenta}{As was done for the closely-fit channel, we use
equation~\eqref{eqn:areaROCdecomposed} to investigate the effects of the
tension and bending forces on semipermeability.
Figure~\ref{fig:contractingRAdecomp} shows the rate of change of the
reduced area due to the tension force (blue), the bending force (red),
and the total membrane force (black). Again, the tension is responsible
for the majority of the semipermeability. However, when compared to the
closely-fit channel example, the bending does have more of a
contribution because of the lower maximum imposed velocity. While we
...
the contraction (green). The vesicle does deflate with a smaller
...
\begin{figure*}[htp]
  \ifTikz
  \input{figures/contractingRAdecomp.tikz}
  \else
  \includegraphics{figures/contractingRAdecomp.pdf}
  \fi
  \caption{\label{fig:contractingRAdecomp} \textcolor{magenta}{The rate
  of change of the reduced area caused by the tension and bending
  forces. The dashed line is the sum of these two contributions. Only
  ...
  Figure~\ref{fig:contractingComposite}(b). The green line corresponds
  to the simulation in Figure~\ref{fig:contractingComposite}(a). (b) The
  high semipermeability coefficient in
  Figure~\ref{fig:contractingComposite}(e). The green line corresponds
